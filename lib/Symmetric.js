...
module.exports = class Symmetric
{
    ...
    static encryptWithAd(plaintext, symKey, aad = "") {
        ...
        let ad;
        if (aad.length >= 1) {
            ad = Buffer.from(HEADER + nonce.toString('binary') + aad.toString('binary'), 'binary');
        } else {
            ad = Buffer.from(HEADER + nonce.toString('binary'), 'binary')
        }
    ...
    static decryptWithAd(ciphertext, symKey, aad = "") {
        ...
        try {
            ...
            console.log({
                "key": symKey.getBuffer().toString('hex'),
                "ad": ad.toString('hex'),
